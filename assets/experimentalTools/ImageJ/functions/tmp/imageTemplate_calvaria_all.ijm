...
// 4 wk calvaria
samples = newArray(2,4,2,2,4,2,4,2,1,1,3,3,3,3,3,3,4,3,2,4,4,4);
noofsamples = 22;
...
Dialog.setInsets(0, 0, 0); 
tagItems = newArray("select", "hem", "thr", "big");
Dialog.addRadioButtonGroup("tag type", tagItems, 1, 4, "select");

Dialog.setInsets(0, 0, 0);
Dialog.addMessage("----------------------------------------------------------------------------"); 
Dialog.setInsets(0, 0, 0); 
Dialog.addMessage("template");
Dialog.addNumber("Number of column", 3);
...
if (TemplateType == "portrailt") {
	width = 500*NoOfColumn*Magnification+20;
	height = 100*NoOfRow*Magnification+200;
	
	cell_width_image = 500*Magnification;
	cell_height_image = 100*Magnification;

	cell_width_title = 500*Magnification;
...
function drawimages(inputList) {
	...
	if (maxsize > cell_width_image*Magnification) {
		cell_minification = maxsize/cell_width_image;
		minify_boolean = true;
	} else {
