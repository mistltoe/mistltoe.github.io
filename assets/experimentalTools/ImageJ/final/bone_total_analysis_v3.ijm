...
Dialog.addMessage("----------------------------------------------------------------------------"); 
Dialog.setInsets(0, 0, 0);
Dialog.addMessage("Paranormic viewer export option, '1:4' mean 2, '1:8' means 1");
Dialog.setInsets(0, 0, 0);
Dialog.addNumber("Magnification ratio", 1, 1, 2, "folds");
...
Dialog.addMessage("----------------------------------------------------------------------------"); 
Dialog.setInsets(0, 0, 0); 
Dialog.addRadioButtonGroup("Analysis which you are", targetTissue, 1, 2, "calvarium");
Dialog.setInsets(0, 0, 0); 
...
userThrMin = Dialog.getNumber();
userThrMax = Dialog.getNumber();
...
if (isThreshold == 1) {
	...
	// Selection area
	sWidth = 1250*magOption;
	sHeight = 1250*magOption;
	...
	// Selection area
	sWidth = 3000*magOption;
	sHeight = 1000*magOption;
...
function bone_Analysis(directory, startOption){
	...
	for (i;i<fl.length;i++) {
		...
		if (analysisOption == "HnE") {
			...
			setTool("polygon");
			waitForUser("Select  area in image");
			...
			run("Draw", "slice");	
			saveAs("Tiff", analysisDir+"/"+expName+"_draw.tif");	
			close(expName+"_draw.tif");
